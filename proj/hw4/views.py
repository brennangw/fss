...
#         post_data = {status: 'accept', id: id}
#         requests.post('localhost:8080/fpml/process-swap/accept', data=post_data) 
...
def EODProcess(request):
    if request.method == 'POST':
        ...
        if str(this_month_expir_date) == str(date):
            month_code = moth_num_to_code(int(month))
            cursor = connection.cursor()
            cursor.execute("SELECT * FROM trade where year=%s and month_code=%s",[year, str(month_code)])
            expiring_contracts = cursor.fetchall() 
            for i in range(len(expiring_contracts)):
                print expiring_contracts[i]
                writer.writerow(expiring_contracts[i])
                
        return response
    return render(request, 'hw4/EOD.html')
...
def addtrader(request):
    ...
    return render(request,'hw4/newTrader.html', context= {'success': success})
...
def addtrade(request):
    ...
    if request.method == 'POST':
       ...
       return render(request, 'hw4/newTrade.html')
...
def aggregate(request):
    ...
    return render(request, 'hw4/aggregate.html')
...
def history(request):
    ...
    return render(request, 'hw4/history.html')
...
def pnl(request):
    ...
    return render(request, 'hw4/pnl.html')
...
def portfolio(request):
    ...
    return render(request, 'hw4/portfolio.html')
...
def pnlbytrade(request):
    ...
    return render(request, 'hw4/pnlbytrade.html')
...
def pnlbytrader(request):
    ...
    return render(request, 'hw4/pnlbytrader.html')
...
def pnlbyproduct(request):
    ...
    return render(request, 'hw4/pnlbyproduct.html')
